...
red for error, purple for analytics. Using a colour outside this system requires explicit
justification.
...
| Token | Where |
|---|---|
| `var(--gold)` | Eyebrow lines and text, accent bars (3px), left-border callouts |
...
When multiple data series need distinct colours in charts, use this sequence in order:
...
Never pick colours outside this series for chart data. Arbitrary colours break the semantic
association between colour and meaning.
...
function KpiCard({ label, value, sub, accent }) {
  return (
    <div style={{ display: 'flex', alignItems: 'stretch', gap: 16, ...card }}>
      ...
      <div>
        <div style={{ fontFamily: 'var(--fd)', fontSize: 30, fontWeight: 300 }}>{value}</div>
...
The left accent bar (`width: 3, height: 100%`) uses a semantic colour per KPI:
primary navy for counts, gold for scores, green for positive deltas, purple for analytics.
...
*Last updated: 2026-05-12. Maintained by Octavio Pérez Bravo.*
